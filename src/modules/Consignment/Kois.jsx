...
const Kois = () => {
  ...
  useEffect(() => {
    const fetchFishCareData = async () => {
      ...
      try {
        const response =
          await ConsignmentApi.getAllHealthCareConsignmentForCustomer(
            id,
            currentPageCare,
            pageSizeCare
          );
  ...
  useEffect(() => {
    const fetchFishSellData = async () => {
      ...
      const id = user?.id;
      try {
        const response = await ConsignmentApi.getAllSellConsignmentForCustomer(
          id,
          currentPageSell,
          pageSizeSell
        );
  ...
  const { mutate: handleFishDetail } = useMutation({
    mutationFn: (id) =>
      ConsignmentApi.getAllHealthCareConsignmentForCustomerDetail(id),
      ...
      children: (
        <Table
          ...
          dataSource={fishCareList.map((fish) => ({
            ...
            careEnvironment:
              fish.koiFish.healthcare?.careEnvironment || "Đang cập nhật",
            healthStatus:
              fish.koiFish.healthcare?.healthStatus || "Đang cập nhật",
            growthStatus:
              fish.koiFish.healthcare?.growthStatus || "Đang cập nhật",
            dayRemain: fish.koiFish.healthcare?.dayRemain || "Đang cập nhật",
            grow:
              fish.koiFish.healthcare?.growthStatus -
              fish.koiFish.healthcare?.lastGrowth,
          }))}
          columns={[
            {
              title: "Mã ký gửi",
              ...
              return (
                <div>
                  <h3 className="text-center font-bold">
                    Chi tiết Lịch sử chăm sóc cho cá {record.name} ID:{" "}
                    {record.id}{" "}
                  </h3>
                  <Table
                    locale={{
                    ...
                    columns={[
                      ...
                      },
                      {
                        ...
                        render: (_, record, index) => {
                          ...
                          const nextGrowthStatus =
                            healthcareData[index + 1].growthStatus;
                          const currentGrowthStatus = record.growthStatus;
                          const differences =
                            currentGrowthStatus - nextGrowthStatus;
                    ...
                    dataSource={healthcareData.map((item, index) => {
                      ...
                      return {
                        ...
                        careEnvironment:
                          item.careEnvironment || record.careEnvironment,
